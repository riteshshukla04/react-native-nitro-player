...
Options for `PlayerQueue.shufflePlaylist`.

```typescript
interface ShufflePlaylistOptions {
  // Default: true — the playing track moves to index 0 and the rest is shuffled behind it.
  // false shuffles the whole list; the playing track keeps playing wherever it lands.
...
interface PlayerConfig {
  ...
  androidAudioFocus?: AndroidAudioFocusMode; // Default: 'pause' — Android only
}
```
...
```typescript
type AndroidAudioFocusMode = 'pause' | 'duck' | 'ignore';
```

| Value | Behaviour |
|---|---|
| `'pause'` | Pause on an interruption, resume after it. Default. |
| `'duck'` | Keep playing at a reduced volume through transient interruptions. |
| `'ignore'` | Never react to audio focus. |
